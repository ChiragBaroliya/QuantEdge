...
Create the systemd service file `/etc/systemd/system/quantedge-worker-history-1m.service`:

```bash
sudo nano /etc/systemd/system/quantedge-worker-history-1m.service
```
...
```ini
[Unit]
Description=QuantEdge Historical 1-Minute Data Gap Sync Service
...
Run the following command in your SSH terminal to start the 1-Minute sync service:
...
3. **Symbol Completion:**
   `Successfully backfilled and updated history stored to 1 for symbol <SYMBOL> (1m).`
4. **Shutdown:**
...
### Manual Verification
- Run `systemctl status quantedge-worker-history-1m` to verify if the service is active, running, or has finished (inactive/dead) successfully.
- Log into your database via PostgreSQL cli (`psql -d quantedge`) and run this query to verify symbols are being processed and updated:
  ```sql
  SELECT symbol, is_histry_stored_1m FROM stock_master;
  ```
