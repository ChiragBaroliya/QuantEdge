# Live Market Data Feed Execution Plan (1d)

This plan details how to deploy, manage, and monitor the continuous, auto-starting live market data feed service for the `1-day` timeframe.

---

## Background Context
Unlike on-demand historical gap syncs, these services run **continuously** (`start=auto`). They establish a live WebSocket connection to Zerodha (Kite Connect) to stream real-time tick/candle feeds, aggregate them into the `1-day` timeframe, and persist the candles to the database.
...
Create the systemd service file `/etc/systemd/system/quantedge-worker-marketdatafeed-1d.service`:

```bash
sudo nano /etc/systemd/system/quantedge-worker-marketdatafeed-1d.service
```
...
```ini
[Unit]
Description=QuantEdge Live Market Data Feed Service (1d)
...
ExecStart=/usr/bin/dotnet QuantEdge.Worker.dll marketdatafeed:1d
Restart=always
RestartSec=5
KillMode=process
...
```bash
sudo systemctl enable quantedge-worker-marketdatafeed-1d
```
...
```bash
# View live logs for 1-Day Feed
sudo journalctl -u quantedge-worker-marketdatafeed-1d -f
```
...
- Check service active state:
  ```bash
  sudo systemctl status quantedge-worker-marketdatafeed-1d
  ```
- Query the database to verify new candles are actively being inserted:
  ```sql
  -- Check latest records in market_candles_1d table
  SELECT * FROM market_candles_1d ORDER BY candle_time DESC LIMIT 5;
  ```
