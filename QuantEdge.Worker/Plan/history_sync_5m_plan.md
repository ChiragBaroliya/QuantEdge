...
Create the systemd service file `/etc/systemd/system/quantedge-worker-history-5m.service`:

```bash
sudo nano /etc/systemd/system/quantedge-worker-history-5m.service
```
...
```ini
[Unit]
Description=QuantEdge Historical 5-Minute Data Gap Sync Service
...
ExecStart=/usr/bin/dotnet QuantEdge.Worker.dll history:5m
Restart=no
KillMode=process
...
sudo systemctl disable quantedge-worker-history-5m
```

---

### Step 3: Trigger the Historical Sync Service
Run the following command in your SSH terminal to start the 5-Minute sync service:
...
1. **Startup:** 
   `Starting QuantEdge.Worker with job: history:5m`
2. **Execution Begin:**
...
3. **Symbol Completion:**
   `Successfully backfilled and updated history stored to 1 for symbol <SYMBOL> (5m).`
4. **Shutdown:**
...
### Manual Verification
- Run `systemctl status quantedge-worker-history-5m` to verify if the service is active, running, or has finished (inactive/dead) successfully.
- Log into your database via PostgreSQL cli (`psql -d quantedge`) and run this query to verify symbols are being processed and updated:
  ```sql
  SELECT symbol, is_histry_stored_5m FROM stock_master;
  ```
