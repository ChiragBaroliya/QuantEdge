# Live Market Data Feed Execution Plan (60m)

This plan details how to deploy, manage, and monitor the continuous, auto-starting live market data feed service for the `60-minute` timeframe.

---

## Background Context
Unlike on-demand historical gap syncs, these services run **continuously** (`start=auto`). They establish a live WebSocket connection to Zerodha (Kite Connect) to stream real-time tick/candle feeds, aggregate them into the `60-minute` timeframe, and persist the candles to the database.
...
Create the systemd service file `/etc/systemd/system/quantedge-worker-marketdatafeed-60m.service`:

```bash
sudo nano /etc/systemd/system/quantedge-worker-marketdatafeed-60m.service
```
...
```ini
[Unit]
Description=QuantEdge Live Market Data Feed Service (60m)
...
ExecStart=/usr/bin/dotnet QuantEdge.Worker.dll marketdatafeed:60m
Restart=always
RestartSec=5
KillMode=process
...
```bash
sudo systemctl enable quantedge-worker-marketdatafeed-60m
```
...
```bash
# View live logs for 60-Minute Feed
sudo journalctl -u quantedge-worker-marketdatafeed-60m -f
```
...
- Check service active state:
  ```bash
  sudo systemctl status quantedge-worker-marketdatafeed-60m
  ```
- Query the database to verify new candles are actively being inserted:
  ```sql
  -- Check latest records in market_candles_60m table
  SELECT * FROM market_candles_60m ORDER BY candle_time DESC LIMIT 5;
  ```
