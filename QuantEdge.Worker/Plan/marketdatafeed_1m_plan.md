# Live Market Data Feed Execution Plan (1m)

This plan details how to deploy, manage, and monitor the continuous, auto-starting live market data feed service for the `1-minute` timeframe.

---

## Background Context
Unlike on-demand historical gap syncs, these services run **continuously** (`start=auto`). They establish a live WebSocket connection to Zerodha (Kite Connect) to stream real-time tick/candle feeds, aggregate them into the `1-minute` timeframe, and persist the candles to the database.
...
Create the systemd service file `/etc/systemd/system/quantedge-worker-marketdatafeed-1m.service`:

```bash
sudo nano /etc/systemd/system/quantedge-worker-marketdatafeed-1m.service
```
...
```ini
[Unit]
Description=QuantEdge Live Market Data Feed Service (1m)
...
ExecStart=/usr/bin/dotnet QuantEdge.Worker.dll marketdatafeed:1m
Restart=always
RestartSec=5
KillMode=process
...
- Query the database to verify new candles are actively being inserted:
  ```sql
  -- Check latest records in market_candles_1m table
  SELECT * FROM market_candles_1m ORDER BY candle_time DESC LIMIT 5;
  ```
