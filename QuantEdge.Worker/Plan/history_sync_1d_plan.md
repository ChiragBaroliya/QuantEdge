...
Create the systemd service file `/etc/systemd/system/quantedge-worker-history-1d.service`:

```bash
sudo nano /etc/systemd/system/quantedge-worker-history-1d.service
```
...
```ini
[Unit]
Description=QuantEdge Historical 1-Day Data Gap Sync Service
...
ExecStart=/usr/bin/dotnet QuantEdge.Worker.dll history:1d
Restart=no
KillMode=process
...
sudo systemctl disable quantedge-worker-history-1d
```

---

### Step 3: Trigger the Historical Sync Service
Run the following command in your SSH terminal to start the 1-Day sync service:

```bash
# Start 1-Day Historical Gap Sync
sudo systemctl start quantedge-worker-history-1d
```
...
```bash
# Monitor 1-Day Gap Sync logs
sudo journalctl -u quantedge-worker-history-1d -f
```

### Key Logs to Watch For:
1. **Startup:** 
   `Starting QuantEdge.Worker with job: history:1d`
2. **Execution Begin:**
...
3. **Symbol Completion:**
   `Successfully backfilled and updated history stored to 1 for symbol <SYMBOL> (1d).`
4. **Shutdown:**
...
### Manual Verification
- Run `systemctl status quantedge-worker-history-1d` to verify if the service is active, running, or has finished (inactive/dead) successfully.
- Log into your database via PostgreSQL cli (`psql -d quantedge`) and run this query to verify symbols are being processed and updated:
  ```sql
  SELECT symbol, is_histry_stored_1d FROM stock_master;
  ```
