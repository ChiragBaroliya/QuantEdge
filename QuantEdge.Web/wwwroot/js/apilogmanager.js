/**
 * API Log Manager & Diagnostic Analyzer
 * QuantEdge Web Frontend Logic - Calls QuantEdge.API Endpoint (/api/log/logs-by-date?startDate=YYYY-MM-DD&endDate=YYYY-MM-DD&category=API)
 */

document.addEventListener('DOMContentLoaded', () => {
    const container = document.querySelector('.log-manager-container');
    const apiBaseUrl = (container ? container.getAttribute('data-api-base-url') : '') || 'https://localhost:44370';
    ...
    // Initialize Page by Fetching API Logs for Selected Date Range
    fetchApiLogsForDateRange(logStartDateInput.value, logEndDateInput.value);
    ...
    if (btnFetchDateLogs) {
        btnFetchDateLogs.addEventListener('click', () => {
            ...
            fetchApiLogsForDateRange(startVal, endVal);
        });
    }
    ...
    // Fetch API Logs via API Project Endpoint /api/log/logs-by-date?startDate=YYYY-MM-DD&endDate=YYYY-MM-DD&category=API
    async function fetchApiLogsForDateRange(startDateStr, endDateStr, targetFileName = null) {
        logFilesList.innerHTML = `
            <div class="file-list-loading">
                <div class="spinner-sm"></div>
                <span>Fetching API logs...</span>
            </div>
        `;

        try {
            let primaryUrl = `/Log/GetApiLogs?startDate=${encodeURIComponent(startDateStr || '')}&endDate=${encodeURIComponent(endDateStr || '')}`;
            if (targetFileName) {
                primaryUrl += `&fileName=${encodeURIComponent(targetFileName)}`;
            }
            ...
            try {
                ...
                let directUrl = `${apiBaseUrl.replace(/\/$/, '')}/api/log/logs-by-date?startDate=${encodeURIComponent(startDateStr || '')}&endDate=${encodeURIComponent(endDateStr || '')}&category=API`;
                if (targetFileName) directUrl += `&fileName=${encodeURIComponent(targetFileName)}`;
            ...
            if (data.success) {
                // Strict API filter: only keep API logs
                availableFiles = (data.files || []).filter(f => 
                    (f.appTag && f.appTag === 'API') || 
                    (f.fileName && f.fileName.toLowerCase().startsWith('api')) ||
                    (f.category && f.category === 'API')
                );

                filterAndRenderFileList();

                if (data.selectedFileName && data.content && isApiFile(data.selectedFileName)) {
                    ...
                    activeFileTag.innerText = fileObj ? fileObj.appTag : 'API LOG';
                    ...
                    renderEmptyFileList(`No API log files found for selected date range.`);
                    showErrorState(`No API log files available for selected date range.`);
                }
            } else {
                renderEmptyFileList(data.message || `No API logs found.`);
                showErrorState(data.message || `Failed to fetch API logs.`);
            }
        } catch (err) {
            console.error("Failed to fetch API logs by date range:", err);
            renderEmptyFileList("Error connecting to API server.");
            showErrorState("Server error reading API log files.");
        }
    }
    ...
    function isApiFile(fileName) {
        ...
        return lower.startsWith('api') || lower.includes('api_log');
    }
        ...
        files.forEach(file => {
            ...
            fileCard.innerHTML = `
                <div class="file-item-header">
                    ...
                    <span class="file-app-tag">${escapeHtml(file.appTag || 'API Log')}</span>
                </div>
    ...
    function renderEmptyFileList(msg = "No API log files found.") {
        logFilesList.innerHTML = `
    ...
    // Select and Read Specific API Log File
    async function selectLogFile(fileName) {
        ...
        try {
            let primaryUrl = `/Log/GetApiLogContent?fileName=${encodeURIComponent(fileName)}`;
            let response;
            try {
                response = await fetch(primaryUrl);
            } catch (netErr) {
                let directUrl = `${apiBaseUrl.replace(/\/$/, '')}/api/log/content?fileName=${encodeURIComponent(fileName)}`;
                response = await fetch(directUrl);
            }
            ...
            if (data.success && data.content) {
                ...
                showErrorState(data.message || 'Failed to read API log file content.');
            }
        } catch (err) {
            console.error("Error fetching API log content:", err);
            showErrorState("Server error reading API log file.");
        }
    }
                ...
                <tr class="empty-row">
                    <td colspan="3">
                        <div class="empty-state">
                            ...
                            <h3>No API Log Entries Found</h3>
    ...
    function exportFilteredLogsAsJson() {
        ...
        anchor.setAttribute("download", `QuantEdge_ApiLog_${currentSelectedFileName || 'export'}.json`);
